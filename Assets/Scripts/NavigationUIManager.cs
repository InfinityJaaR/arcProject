...
public class NavigationUIManager : MonoBehaviour
{
    ...
    public async void LoadAvailableLocations()
    {
        ...
        foreach (BuildingData building in allBuildings)
        {
            ...
            {
                filteredCount++;
                Debug.Log($"[NavigationUIManager] ⏭️ '{building.name}' filtrado (type='{building.type}')");
            }
        }
        
        Debug.Log($"[NavigationUIManager] 📊 Total cargados: {allBuildings.Count}, Edificios navegables: {availableBuildings.Count}, Filtrados: {filteredCount}");
    ...
    private void PopulateLocationList()
    {
        ...
        foreach (BuildingData building in availableBuildings)
        {
            ...
            // Configurar texto del botón - SOLO NOMBRE
            TextMeshProUGUI buttonText = buttonObj.GetComponentInChildren<TextMeshProUGUI>();
            if (buttonText != null)
            {
                // Solo mostrar el nombre en grande, sin descripción
